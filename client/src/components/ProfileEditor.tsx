...
import { Camera, MapPin, Briefcase, Heart, Wand2, Loader2, Save, ScanFace, Image as ImageIcon } from 'lucide-react';
...
const ProfileEditor: React.FC<ProfileEditorProps> = ({ profile, setProfile }) => {
  ...
  const [optInPhotos, setOptInPhotos] = useState(false);
  ...
  const runAiAnalysis = async () => {
    ...
    try {
      const result = await analyzeProfile(profile, optInPhotos);
      ...
      {(showAnalysis || isAnalyzing) && (
        <>
          ...
          <AnalysisPanel
            ...
            onApplySuggestion={(category, value) => {
              if (category.toLowerCase() === 'bio') {
                setProfile({ ...profile, bio: value });
              } else if (category.toLowerCase() === 'interests') {
                // Split by comma if multiple interests are suggested
                const newInterests = value.split(',').map(i => i.trim()).filter(i => i.length > 0);
                // Add only unique interests
                const currentInterests = new Set(profile.interests);
                newInterests.forEach(i => currentInterests.add(i));
                setProfile({ ...profile, interests: Array.from(currentInterests) });
              }
            }}
          />
        </>
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <label className="flex items-center gap-2 text-xs text-indigo-200 hover:text-white cursor-pointer transition-colors">
                    <input
                      type="checkbox"
                      checked={optInPhotos}
                      onChange={(e) => setOptInPhotos(e.target.checked)}
                      className="rounded border-indigo-400 bg-indigo-900/50 text-rose-500 focus:ring-rose-500"
                    />
                    <span>Allow AI to analyze my photos</span>
                  </label>
                </div>
              </div>
